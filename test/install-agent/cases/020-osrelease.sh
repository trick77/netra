...
# detection. These are unit tests: install-agent.sh is sourced with
...
# Below-floor and unrecognised distros both land on `unknown`: the installer has
...
# backticked NAME. os-release is untrusted input the installer is validating, so
...
NETRA_INSTALL_ROOT=$(fixture root-debian12)
...
NETRA_INSTALL_ROOT=$(fixture root-rocky9)
export NETRA_INSTALL_ROOT
init_paths
