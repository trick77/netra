...
#
# These are unit tests: install-agent.sh is sourced with NETRA_SOURCED=1 so the
...
assert_not_contains "$NETRA_BLK_VOLUMES" "$ROOT" \
    "NETRA_INSTALL_ROOT never leaks into an emitted source path"
