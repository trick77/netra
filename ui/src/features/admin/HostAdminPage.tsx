 ...
 *
 * Bundled into one options object rather than five positional parameters,
...
export function HostAdminPage() {
  ...
  async function onCreate() {
    ...
    setFormError(null);
    try {
      const created = await createHost(hostname.trim());
      setMinted({
    ...
    <>
      ...
      {hosts !== null && hosts.length > 0 ? (
        <>
          ...
          {/* The shared Table rather than hand-rolled markup: hand-rolling it
              also hand-rolls away the sorting. */}
          <Table
            columns={hostColumns({
              rotating,
              confirming,
