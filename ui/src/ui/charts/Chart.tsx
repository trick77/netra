  ...
  return (
    <>
      {Array.from({ length: Math.ceil(series.length / 2) }, (_, p) => {
        ...
        // plot, not from the call site: the same component draws a 175px
        // fleet cell at more than one point per pixel and a 1000px dialog at
        // three and a half, and only one of those can carry a 1.25px outline
        // without the outline becoming the mark. See mirrorEdge().
        //
        // There is no per-caller override of this. The fleet cell carried one
        // for a while -- a line with a light fill, so the mirror would read as
        // one of a row of silhouettes -- and it made the traffic cell a
        // translucent wash where every reference draws a filled area. A
        // mirrored pair is a MASS at every density it is drawn at; how heavy
        // that mass is, is a property of the plot and is answered here.
